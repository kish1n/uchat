...
void send_register_request(const char *url, const char *username, const char *password) {
    ...
    if (!uri) {
        g_printerr("Помилка парсинга URL: %s\n", error->message);
        goto cleanup;
    }
    ...
    if (!connection) {
        g_printerr("Помилка з'єднання: %s\n", error->message);
        goto cleanup;
    }
    ...
    if (error) {
        g_printerr("Помилка отправки данных: %s\n", error->message);
        goto cleanup;
    } else {
        g_print("Запит відправлен успешно.\n");
    }
    ...
    while (TRUE) {
        ...
        if (bytes_read > 0) {
            ...
            g_printerr("Помилка читання ответа: %s\n", error->message);
            g_clear_error(&error);
            break;
        }
    }
