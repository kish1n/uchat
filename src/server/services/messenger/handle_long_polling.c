...
int handle_long_polling(HttpContext *context, struct MHD_Connection *connection, int chat_id) {
    if (!context || chat_id <= 0) {
        logging(ERROR, "Invalid context or chat_id passed to handle_long_polling");
        return MHD_NO;
    }
    ...
    int elapsed_time = 0;
    // int last_message_id = get_last_message_id(db) ;
    //
    // while (elapsed_time < MAX_POLL_TIME) {
    //     Message *messages = NULL;
    //     int message_count = 0;
    //
    //     if (get_new_messages_by_chat(context->db_conn, chat_id, last_message_id, &messages, &message_count) == 0) {
    //         if (message_count > 0) {
    ...
    logging(INFO, "Long polling completed for chat ID: %d", chat_id);
    free_user(sender);
    return ret;
}
