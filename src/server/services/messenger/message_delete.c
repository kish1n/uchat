...
int handle_delete_message(HttpContext *context) {
    ...
    *context->con_cls = NULL;
    ...
    if (delete_message(context->db_conn, message_id) != 0) {
        logging(ERROR, "Failed to delete message with ID %d", message_id);
        int ret = prepare_simple_response("Failed to delete message", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
