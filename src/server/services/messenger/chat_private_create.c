...
int handle_create_private_chat(HttpContext *context) {
    ...
    *context->con_cls = NULL;
    ...
    if (second_user == NULL || creator == NULL) {
        ...
        return prepare_response("Failed to get user by username", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
    }
    ...
    if (chat_id == -1) {
        ...
        return prepare_response("Failed to check if private chat exists", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
    } else if (chat_id > 0) {
        logging(ERROR, "Private chat between '%s' and '%s' already exists", creator->username, second_user->username);
        return prepare_response("already exists", STATUS_CONFLICT, parsed_json, context);
    }
    ...
    if (!chat_name) {
        ...
        int ret = prepare_response("Failed to allocate memory for chat name", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
    ...
    if (chat_id <= 0) {
        ...
        int ret = prepare_response("Failed to create chat", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
    ...
    if (add_chat_member(context->db_conn, chat_id, creator->id, 1) != 0) {
        ...
        int ret = prepare_response("Failed to add chat creator", STATUS_INTERNAL_SERVER_ERROR, parsed_json, context);
    ...
    int ret = prepare_response("Chat created successfully", STATUS_OK, NULL, context);
